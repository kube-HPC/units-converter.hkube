...
Units.initEnum(['Ki', 'M', 'Mi', 'Gi', 'm', 'K', 'G', 'T', 'Ti']);
...
const getMemoryInBytes = (memory, allowBytes) => {
    if (!allowBytes) {
...
const getMemoryInKi = (memory, allowBytes) => {
    if (!allowBytes) {
        _validateMemory(memory);
    }
...
const getMemoryInMi = (memory, allowBytes) => {
    if (!allowBytes) {
        _validateMemory(memory);
    }
